      ...
      <div style="margin-top: 15px">
        <el-form :inline="true"
          ...
          <el-form-item label="订单来源：">
            <el-select v-model="listQuery.sourceType"
                       class="input-width"
                       placeholder="全部"
                       clearable>
      ...
      <el-table ref="orderTable"
        ...
        <el-table-column label="操作"
          ...
          <template slot-scope="scope">
            <el-button size="mini"
                       @click="handleViewOrder(scope.$index, scope.row)">查看</el-button>
            <el-button size="mini"
                       @click="handleCloseOrder(scope.$index, scope.row)"
                       v-show="scope.row.status===101">关闭</el-button>
            <el-button size="mini"
                       @click="handleDeliveryOrder(scope.$index, scope.row)"
                       v-show="scope.row.status===201">发货</el-button>
            <el-button size="mini"
                       @click="handleViewLogistics(scope.$index, scope.row)"
                       v-show="scope.row.status===201||scope.row.status===301">跟踪</el-button>
            <!-- <el-button
              size="mini"
              type="danger"
    ...
    <el-dialog title="关闭订单"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="handleCloseOrderConfirm">确 定</el-button>
      </span>
    </el-dialog>
    <logistics-dialog v-model="logisticsDialogVisible"
                      ref="logisticsDialog"></logistics-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleViewLogistics (index, row) {
      ...
      this.$refs.logisticsDialog.loadTracking(row.deliveryCompanyId, row.deliverySn);
